...
const float coeffs_lp[N] = {0.25, 0.25, 0.25, 0.25};
const float coeffs_hp[N] = {0.5, -0.5, 0.5, -0.5};
const float coeffs_bp[N] = {0, 0.5, 0.5, 0};

__attribute__((sdx_kernel("filter", 0))) void filter(float sample_in, float* sample_out, ap_uint<2> filter_type) {
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=filter
# 19 "filter.cpp"
...
#pragma HLS BIND_STORAGE variable=buffer type=ram_1p impl=bram
 ...
 VITIS_LOOP_29_1: for (int i = N - 1; i > 0; i--) {
    ...
    float acc = 0;
    VITIS_LOOP_44_2: for (int i = 0; i < N; i++) {
